...
/* must identical to surfaceflinger/DisplayDevice */
//#define CONTAINER_LAYER_STACK  0xFFFF0000
#define CONTAINER_LAYER_STACK  0x00000000
#define NO_LAYER_STACK         0xFFFFFFFF
...
static const uint32_t kMaxTimeLimitSec = 360;       // 3 minutes
...
static enum {
    FORMAT_MP4, FORMAT_H264, FORMAT_FRAMES, FORMAT_RAW_FRAMES, FORMAT_VMI_SVMP
...
static status_t prepareEncoder(float displayFps, sp<MediaCodec>* pCodec,
    ...
    if (gVerbose) {
        ALOGD("Configuring container %d virtual display for %dx%d %s at %.2fMbps\n",
              getContainerId(), gVideoWidth, gVideoHeight, kMimeTypeAvc, gBitRate / 1000000.0);
    }
    ...
    looper->setName("container_virtual_display_looper");
    looper->start();
...
static status_t setDisplayProjection(const sp<IBinder>& dpy,
    ...
    if (gVerbose) {
        if (gRotate) {
            ALOGI("Rotated content area is %ux%u at offset x=%d y=%d\n",
                  outHeight, outWidth, offY, offX);
        } else {
            ALOGI("Content area is %ux%u at offset x=%d y=%d\n",
...
static status_t prepareVirtualDisplay(const DisplayInfo& mainDpyInfo,
    ...
    sp<IBinder> dpy = SurfaceComposerClient::createDisplay(
            String8::format("Container %d Virtual Display", getContainerId()),
	    true /*secure*/);
    ...
    SurfaceComposerClient::setDisplayLayerStack(dpy,
            CONTAINER_LAYER_STACK | getContainerId());
...
static status_t recordScreen(const char* fileName) {
    ...
    if (gVerbose) {
        ALOGI("Main display is %dx%d @%.2ffps (orientation=%u)\n",
    ...
    sp<VDFrameOutput> vdFrameOutput;
    sp<IGraphicBufferProducer> encoderInputSurface;
    if (gOutputFormat != FORMAT_FRAMES && gOutputFormat != FORMAT_RAW_FRAMES &&
        gOutputFormat != FORMAT_VMI_SVMP) {
        ...
        if (err != NO_ERROR && !gSizeSpecified) {
            ...
            if (gVideoWidth != newWidth && gVideoHeight != newHeight) {
                ...
                ALOGW("WARNING: failed at %dx%d, retrying at %dx%d\n",
    ...
    } else if(gOutputFormat == FORMAT_FRAMES || gOutputFormat == FORMAT_RAW_FRAMES){
    ...
    } else { // FORMAT_VMI_SVMP
        vdFrameOutput = new VDFrameOutput(getContainerId());
        err = vdFrameOutput->createInputSurface(gVideoWidth, gVideoHeight, &encoderInputSurface);
        if(err != NO_ERROR) {
            return err;
        }
    }
    ...
    if (gWantFrameTime) {
        ...
        if (gVerbose) {
            ALOGI("Bugreport overlay created\n");
        }
    } else {
    ...
    if (gOutputFormat == FORMAT_FRAMES || gOutputFormat == FORMAT_RAW_FRAMES) {
    ...
    } else if(gOutputFormat == FORMAT_VMI_SVMP)    {
        while (!gStopRequested) {
            err = vdFrameOutput->copyFrame(250000);
            if (err == ETIMEDOUT) {
                err = NO_ERROR;
            } else if (err != NO_ERROR) {
                ALOGE("Got error %d from copyFrame()", err);
        ...
        if (gVerbose) {
            ALOGI("Stopping encoder and muxer\n");
        }
    }
    ...
    if (muxer != NULL) {
        ...
        err = muxer->stop();
    } else if (rawFp != NULL && rawFp != stdout) {
        fclose(rawFp);
    }
    if (encoder != NULL) encoder->release();
 ...
 */
static bool parseWidthHeight(const char* widthHeight, uint32_t* pWidth, uint32_t* pHeight) {
    long width, height;
    char* end;
...
static void usage() {
    fprintf(stderr,
        "Usage: con-vd [options] <filename>\n"
        "\n"
        "ICL Container-VD v%d.%d.  Records the device's display to a .mp4 file.\n"
        "\n"
        "Options:\n"
        "--size WIDTHxHEIGHT\n"
...
int main(int argc, char* const argv[]) {
    ...
    while (true) {
        ...
        case 'o':
            ...
            } else if (strcmp(optarg, "vmi-svmp") == 0) {
                gOutputFormat = FORMAT_VMI_SVMP;
            } else {
